...
const designTools = new DesignTools();

const NavButtons = {
	home:     new Button({type: "nav", innerText: "Home"}),
	about:    new Button({type: "nav", innerText: "About"}),
	projects: new Button({type: "nav", innerText: "Projects"}),
	resume:   new Button({type: "nav", innerText: "Resume"}),
};
...
function setupDesignTools() {
	designTools.drawWheel();
	body.appendChild(designTools.el);
}

function setupFooter() {
	for (const [name, button] of Object.entries(FooterButtons)) {
		footer.el.appendChild(button.el);
	}
